...
class AuthService
{
    ...
    public function getProfile(Request $request): JsonResponse
    {
        ...
        $userResource = UserResource::make($this->user->load('university'));
        ...
        return success(
            ...
            [
                ...
                'notifications_count' => $this->notificationService->getAllNotifications($request, 0, true),
            ]
        );
    }
    ...
    public function login(LoginRequest $request, $isAdmin = false)
    {
        ...
        if ($request->has('university_number')) {
            $userQuery->where('university_number', $request->university_number);
        } else {
            $userQuery->where('name', $request->name);
        }

        $user = $userQuery->$fun('roles', function ($q) {
            $q->where(function ($query) {
                $query->where('name', Constants::ADMIN_ROLE)
                    ->orWhere('name', Constants::SUPER_ADMIN_ROLE)
                    ->orWhere('name', Constants::PROJECT_MANAGER_ROLE)
                    ->orWhere('name', Constants::CONTENT_MANAGER_ROLE);
            });
        })->first();
        if (!$user)
